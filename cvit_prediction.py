...
filenames = sorted(
    [x for x in os.listdir(sample) if x[-4:] == ".mp4"]
...
checkpoint = torch.load(
    "weight\deepfake_cvit_gpu_ep_50.pth", map_location=torch.device("cpu")
)
    ...
    if count < 5 and (mtcnn_frame[0] is not None):
        for face_mt in mtcnn_frame[0]:
            ...
            if face_mt.size > 0 and (count < 5):
                resized_image_mtcnn = cv2.resize(
                    face_mt, (224, 224), interpolation=cv2.INTER_AREA
                )
                resized_image_mtcnn = cv2.cvtColor(
                    resized_image_mtcnn, cv2.COLOR_RGB2BGR
                )
...
def face_blaze(video_path, filename, face_tensor_blaze):
    ...
    for frame_data in faces:
        for face in frame_data["faces"]:
            if count_blaze < 44:
                resized_facefrm = cv2.resize(
                    face, (224, 224), interpolation=cv2.INTER_AREA
                )
