...
parser.add_option(
    ...
    type="string",
    dest="session",
    help="Training session. Use g for GPU, t for TPU.",
)
...
def train_tpu(model, criterion, optimizer, scheduler, num_epochs, min_val_loss):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "validation"]:
            ...
            phase_idx = 0
            # Iterate over data.
            for inputs, labels in pl.ParallelLoader(
                ...
                if phase_idx % 100 == 0:
                    ...
                    print(
                        "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                            epoch,
                            phase_idx * batch_size,
                            dataset_sizes[phase],
                            100.0 * phase_idx * batch_size / dataset_sizes[phase],
                            loss.item(),
                        )
                    )
                phase_idx += 1
...
if cession == "t":
    train_tpu(
        model, criterion, optimizer, scheduler, num_epochs, min_val_loss
    )  # Train using TPU.
else:
    train_gpu(
        model, criterion, optimizer, scheduler, num_epochs, min_val_loss
